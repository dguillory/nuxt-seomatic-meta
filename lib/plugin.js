  ...
  app.seomaticMeta = async function nuxtSeomaticMeta({ fullPath }, id = null) {
    ...
    const {
      ...
      (acc, [key, value]) => {
        ...
        // swap domains
        if (typeof value == 'string' && options.imgixDomain && options.nativeImgDomain) {
          const replaceString = options.imgixDomain.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
          const replaceRegex = new RegExp(replaceString, 'g');
          value = value.replace(replaceRegex,options.nativeImgDomain);
        }
